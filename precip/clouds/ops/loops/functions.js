...
function getURL(domain,variable,frame,level,season,validtime,year,month,model){
	...
	newurl = newurl.replace("Q",frame);
        ...
        newurl = newurl.replace("MDL",model);
	return newurl;
}
...
function preload(obj){
	...
	for (i2=0; i2<hrs_loop.length; i2++){
		var i1 = hrs_loop[i2];
		var i = frames[i1];
		var urls = getURL(obj.domain,obj.variable,i,obj.level,obj.season,obj.validtime,obj.year,obj.month,obj.model);
...
function pressDown(){
	var curVar = searchByName(imageObj.model,models);
	if(curVar < models.length-1){curVar += 1; changeLevel(models[curVar].name);}
}

function pressUp(){
	var curVar = searchByName(imageObj.model,models);
	if(curVar > 0){curVar = curVar - 1; changeLevel(models[curVar].name);}
}
...
function updateMobile(){
	...
	|| navigator.userAgent.match(/Windows Phone/i)
		...
		document.getElementById('page-middle').innerHTML = "Swipe Left/Right on Image = Change forecast lead | Swipe Up/Down on Image = Change model";
	}
